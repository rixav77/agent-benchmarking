...
- [x] Generate comparison results with confidence intervals
- [x] **Sanity check**: Verify question sets are identical, eval config is identical

**Results (1000 questions, full 3-tier with judge + 95% CIs):**
  - Simple RAG:  EM=0.514, F1=0.607, Combined=0.691, Unanswerable=56.2%
  - Agentic RAG: EM=0.519, F1=0.626, Combined=0.720, Unanswerable=61.2%
  - Agentic wins: combined accuracy (+2.9%), unanswerable detection (+8.9% on subset, CI non-overlapping → significant)
  - Simple wins: answerable EM (+8.6%), answerable F1 (+2.1%) — better at direct extraction
  - EM delta NOT statistically significant (CIs overlap); unanswerable detection IS significant
  - Tier distribution (Simple):  tier1=623 (62.3%), tier2=4, tier3=64, none=309
  - Tier distribution (Agentic): tier1=641 (64.1%), tier2=5, tier3=74, none=280
  - Judge faithfulness: Simple=422 faithful/4 partial/11 unfaithful, Agentic=424/6/9
  - Latency: Agentic 5.46x slower (p50: 3401ms vs 465ms)
  - Tokens:  Agentic 3.51x more (mean 3725 vs 1060)
  - Retrieval (shared BM25): hit_rate@5=0.858, MRR=0.780

**Files saved (gitignored, on disk):**
  - evaluation/predictions/{simple,agentic}_rag_predictions.jsonl (1000 each)
  - evaluation/results/{simple,agentic}_rag_results.json (full per-question, ~650KB each)
  - evaluation/results/{simple,agentic}_rag_results_summary.json
  - evaluation/results/comparison.json (full comparison with 95% CIs)
